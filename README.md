...
- 推送：如有变更，自动提交 `mirror/ dist/ MIRROR_UPDATED` 等文件并推送到当前仓库。

### 使用/定制
1. 打开 GitHub Actions，允许计划任务运行。
2. 默认使用 `scripts/fetch.py` + `scripts/build.py`：前者把源规则同步到本地镜像，后者将模板中的上游 URL 替换成镜像 URL（默认指向 gh-pages 根）。
3. 如不用 GitHub Pages，请在 `scripts/build.py` 中把 `MIRROR_PREFIX` 改为你的镜像地址（示例已在文件内给出）。
4. 手动执行：
   ```bash
